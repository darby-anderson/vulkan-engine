...
Camera::Camera(const glm::vec3& position, const glm::vec3& target, const glm::vec3& up, float near, float far,
               float aspect) :
    position_(position),
    up_(up),
    target_(target),
    orientation_(glm::lookAt(position_, target_, up_)),
    nearP_(near),
    farP_(far),
    aspect_(aspect) {
    projectMatrix_ = glm::perspective<double>(glm::radians(fov_), aspect_, nearP_, farP_);
    ASSERT(!glm::all(glm::isnan(orientation_)), "Orientation incorrect");
}
